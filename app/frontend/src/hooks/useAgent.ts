...
export function useAgent(options: UseAgentOptions = {}) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onmessage = (event) => {
        // Message can be SDK messages or WebSocket control messages (connected, error)
        const rawMessage = JSON.parse(event.data) as
          | SDKMessage
          | WSConnectedResponse
          | (WSErrorResponse & { session_id?: string });

        // Handle control message: connected (must be first to allow type narrowing)
        if (rawMessage.type === 'connected') {
        ...
        // From here on, message is an SDKMessage
